...
public protocol PageFlipTransitionDelegate {
    /// Provide the matched view from the current object's own view hierarchy for the match transition
    func matchedViewFor(transition: PageFlipTransition, otherView: UIView) -> UIView?

    /// Can be used to customize the transition and add extra animation to the animator
    func pageFlipTransitionWillBegin(transition: PageFlipTransition)
}
...
///
public class PageFlipTransition: InteractiveTransition {
    ...
    public private(set) var foregroundContainerView: PageFlipContainerView?
    ...
    public override func canTransitionSimutanously(with transition: Transition) -> Bool {
        transition is PushTransition || transition is PageFlipTransition
    }
    ...
    public override func setupTransition(context: any TransitionContext, animator: TransitionAnimator) {
        ...
        let foregroundDelegate = foreground as? PageFlipTransitionDelegate
        let backgroundDelegate = background as? PageFlipTransitionDelegate

        let overlayView = BlurOverlayView()
        let foregroundContainerView = PageFlipContainerView()
        ...
        backgroundDelegate?.pageFlipTransitionWillBegin(transition: self)
        foregroundDelegate?.pageFlipTransitionWillBegin(transition: self)
    ...
    func setupAnimation(context: any TransitionContext, animator: TransitionAnimator) {
        ...
        animator[foregroundContainerView, \PageFlipContainerView.progress].dismissedValue = 0
        animator[foregroundContainerView, \PageFlipContainerView.progress].presentedValue = 1
...
extension PageFlipTransition: UIGestureRecognizerDelegate {
...
public class PageFlipContainerView: ShadowContainerView {
    var progress: CGFloat = 0 {
        didSet {
            let rotation = Motion.EasingFunction.easeInOut.solveInterpolatedValue(-.pi...0, fraction: progress)
            let translateX = sin(progress * .pi) * 200
            layer.transform = .identity.translatedBy(z: progress * 1000).withPerspective(m34: 1 / 1000).translatedBy(x: translateX).rotatedBy(y: rotation)
            contentView.transform = .identity.scaledBy(x: progress > 0.5 ? 1 : -1)
        }
    }
}
